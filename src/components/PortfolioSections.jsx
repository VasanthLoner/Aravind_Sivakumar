...
import { achievements, certifications, competencies, education, experience, professionalSkills, recognitions, softSkills } from '../data/portfolio'
...
function PortfolioSections() {
  useEffect(() => {
    const items = document.querySelectorAll('.experience-item, .education-item, .certification-item, .recognition-item')
  ...
  return (
    <div>
      ...
      <section className="recognitions section-pad"><div className="section-kicker">06 / Recognition</div><div className="recognition-content"><h2>Awards & <em>recognition.</em></h2><div className="recognition-list">{recognitions.map(([period, title, description]) => <article className="recognition-item" key={title}><div className="recognition-node" aria-hidden="true" /><div><p className="period">{period}</p><h3>{title}</h3><p>{description}</p></div></article>)}</div></div></section>
      <section className="capabilities section-pad"><div className="section-kicker">07 / Competencies</div><div className="competency-content"><h2>Core <em>competencies.</em></h2><div className="capability-grid">{competencies.map((item, index) => <div className="capability reveal" key={item}><span>0{index + 1}</span><p>{item}</p></div>)}</div></div></section>
